...
public class GameUtils
{
  ...
  public static List printMap(String haoMap, String qiHao, String haoMa, String clientweizhi, char[] numHot, String clientHaoMa)
  {
    String iszhong = "未中奖！";
    ...
    for (int i = 0; i < haoma.length; i++)
    {
      ...
      if (clientHaoMa.indexOf(numHot[i]) >= 0)
      {
        ...
        System.out.print("  " + wan + " " + qian + " " + bai + " " + shi + 
          " " + ge + "      ");
        res.add(wan + qian + bai + shi + ge);

        fengeflag += 1;
        if (i == haoma.length - 1) (fengeflag % 70);
  ...
  public static List printMapSort(String haoMap, ArrayList<GameResult> gameResult, String clientWeizhi, String clientHaoMa)
  {
    ...
    for (int i = qiHaos.size() - 1; i >= 0; i--) {
      l.add(printMap(haoMap, (String)qiHaos.get(i), (String)haoMas.get(i), clientWeizhi, 
        numHot, clientHaoMa));
    }
